...
## 선택: Personal Wiki 내부 LLM Namespace

이미 personal wiki repo를 여러 서버/agent에 붙여 쓰고 있다면 별도 `llm-wiki` repo를 모든 서버에 추가하지 않아도 된다. 이 경우 같은 repo 안에 `llm` namespace를 만들고 trust/path boundary를 분리한다.
...
└── wiki/
    ...
    │   └── llm/          # 빠른 원천 메모, 낮은 신뢰
    ├── generated/
    │   └── llm/          # agent가 만든 codebase/ops compile 초안
    ├── reviewed/
    │   └── llm/          # 사람이 검토한 agent/codebase/ops 지식
    └── canonical/        # 안정 정책과 agent entry point
```

이 방식은 repo를 하나만 동기화하면서도 개인 메모와 agent-facing 운영 지식의 신뢰 경계를 유지한다. 상세 판단 기준은 [Repo Topology: Separate Repo vs Internal Namespace](../04-patterns/10-repo-topology-separate-vs-internal.md)를 본다.
